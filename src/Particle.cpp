...
void Particle::init(float x, float y, int  cMode) {
    colorMode = cMode;
...
void Particle::setColorMode(int cMode)
{
    colorMode = cMode;

}

void Particle::updateVertexArrayColors( bool drawingFluid, const ofVec2f &invWindowSize, int i, float* posBuffer, float* colBuffer) 
    {
        int ci = i * 6;
        //	if( drawingFluid ) {
...
#define VMAX 0.013f
        ...
        ofColor color;
		color.setHsb(0,
                     v2 * 255.0f / ( VMAX * VMAX ) + satInc,
                     ofLerp(0.5, 1, mass) * alpha * 255.0f
                     );
        ...
        switch (colorMode)
        {
            case 0: // Red
                ...
                colBuffer[ci++] = (float)0; //color.g;
                colBuffer[ci++] = (float)0; //color.b;
            ...
            case 4: // White/Blue
                ...
                colBuffer[ci++] = (float)0; //color.r;
                colBuffer[ci++] = (float)0; //color.g/2;
                colBuffer[ci++] = color.b;
                break;
                ...
                //            colBuffer[ci++] = color.b;
                //
                //            colBuffer[ci++] = 0; //color.r;
                //            colBuffer[ci++] = 0; //color.g/2;
                //            colBuffer[ci++] = color.b;
                //            break;
                
            case 6: // Light/Dark Green
                colBuffer[ci++] = 0; //color.r;
                colBuffer[ci++] = color.g;
                colBuffer[ci++] = 0; //color.b;
...
void Particle::updateVertexArrays( bool drawingFluid, const ofVec2f &invWindowSize, int i, float* posBuffer, float* colBuffer) {
    ...
    updateVertexArrayColors( drawingFluid, invWindowSize, i,  posBuffer,  colBuffer);
